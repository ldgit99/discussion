...
| 주차 | 핵심 산출물 | 사용자가 할 수 있는 것 |
|------|------------|-------------------|
| **1주차** | 프로젝트 골격 + Supabase Auth + DB 스키마 + RLS + **Session 일괄 모둠 생성** | 교사/학생 가입·로그인, **한 수업 만들면 모둠 N개 코드 자동 발급**, 학생은 코드로 입장 |
| **2주차** | 실시간 팀 채팅 + 의견 카드 + 공동 보드 (기본) | 모둠원이 의견을 등록·공유, 보드에서 비교·조정 |
| **3주차** | AI 진행자 + 핵심 보조 기능 4종 | AI가 토의를 진행하고 요약·비교·근거·합의 보조 |
| **4주차** | 개인 채팅 + 코칭 + 나머지 기능 + 교사 대시보드 완성 | 개인 모드 사고 확장, 교사가 모둠별 결과를 실시간 확인·종합 |
